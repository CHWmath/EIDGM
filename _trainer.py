...
class Trainer:
    def __init__(self, name, prms, device):
        ...
        self.center_penalty = self.prms_GAN['center_penalty'] # add distance penalty term to loss_G
    ...
    def train_G(self, X_data_onehot):
        ...
        loss = loss1 + (int(self.center_penalty)*1e-4)*torch.mean(torch.linalg.norm(p_output-self.center,2))
        loss.backward()
    ...
    def train_gan(self, modes_noisy, X_data, X_data_onehot, EPOCHS=100000, T=5000, iter_D=5, iter_print=100, iter_plot=1000, iter_save=5000):
        ...
        for t in range(1000):
            self.g_model.train()
            self.d_model.eval()
            noise = torch.normal(mean=torch.zeros([1000, self.noise_dim]), std=1.0).to(self.device)
            g_output = self.modify_gen(self.g_model(noise))
            target = (torch.rand(1000, self.num_p)*(self.pscale[1:,:]-self.pscale[:1,:])+self.pscale[:1,:]).to(self.device)
